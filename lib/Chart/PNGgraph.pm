...
$Chart::PNGgraph::needs_binmode = $OS=~/^(WINDOWS|VMS|OS2)/;
...

{
    ...
    sub new  # ( width, height ) optional;
	{
        ...
        if (@_) 
		{
            ...
            die "Usage: Chart::PNGgraph::<type>::new( [x_size, y_size] )\n" unless @_;
            $self->{pngy} = shift;
        } 
		else 
		{
	...
	$s->{_set_error} = 0;
		...
		{ 
		  ...
		  if ($_ eq "pngx" || $_ eq "pngy") {
		    $s->Error("Attempt made to set read-only attribute '$_'-- not set");
		    $s->{_set_error} = 1;
		    next;
		  }
		  else {
    ...
    sub plot_to_png # ("file.png", \@data)
	{
		...
		binmode PNGPLOT if ($Chart::PNGgraph::needs_binmode);
    ...
    sub put_logo($) # GD::Image
	{
		...
		binmode(PNGLOGO) if ($Chart::PNGgraph::needs_binmode);
...
} # End of package Chart::PNGgraph

$Chart::PNGgraph::prog_name;

__END__

=head1 NAME

Chart::PNGgraph - Graph Plotting Module for Perl 5
...
B<Chart::PNGgraph> is a I<perl5> module to create and display PNG output 
for a graph.
The following classes for graphs with axes are defined:

=over 4

=item C<Chart::PNGgraph::lines>

Create a line chart.

=item C<Chart::PNGgraph::bars>

Create a bar chart.

=item C<Chart::PNGgraph::points>

Create an chart, displaying the data as points.

=item C<Chart::PNGgraph::linespoints>

Combination of lines and points.

=item C<Chart::PNGgraph::area>

Create a graph, representing the data as areas under a line.

=item C<Chart::PNGgraph::mixed>
...
I<Chart::PNGgraph> will complain and refuse to compile the graph.
...
use B<undef>, and I<Chart::PNGgraph> will skip that point.

Create a new I<Chart::PNGgraph> object by calling the I<new> operator on the
graph type you want to create (I<chart> is I<bars, lines, points,
linespoints> or I<pie>).

    $my_graph = new Chart::PNGgraph::chart( );
...
=item new Chart::PNGgraph::chart([width,height])
...
L<Chart::PNGgraph::colour> (C<S<perldoc Chart::PNGgraph::colour>> for the names available).
...
the data points, or I<Chart::PNGgraph> will die with a message.
...
First of all: Chart::PNGgraph does B<not> support numerical x axis the way it
...
C<x_tick_number> is set to a defined value, Chart::PNGgraph will attempt to
...
If set to I<'auto'>, Chart::PNGgraph will attempt to format the X axis in a
nice way, based on the actual X values. If set to a number, that's the
number of ticks you will get. If set to undef, Chart::PNGgraph will treat X
data as labels.
Default: undef.
...
package L<Chart::PNGgraph::colour>. C<S<perldoc Chart::PNGgraph::colour>> should give
